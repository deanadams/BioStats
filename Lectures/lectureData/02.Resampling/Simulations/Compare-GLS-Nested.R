## GLS Nested model (with additional factor)
...
result <- t(sapply(1:nsims, function(j){
  ...
  names(x) <- names(x2) <- names(x3) <- names(e)
  y <- (x * b1) + (x2 * b2) + (x3 * b3) + e 
  x <- factor(x); x2 <- factor(x2); x3 <- factor(x3)
  df <- data.frame(x = x, x2 = x2, x3 = x3, y = y, species = names(x))

  fit.RRPP <- lm.rrpp(y ~ x + x2/x3, data = df, Cov = Cov,print.progress = FALSE, SS.type = "III")
  fit.FRPP <- lm.rrpp(y ~ x + x2/x3, data = df, Cov = Cov,
                      RRPP = FALSE, print.progress = FALSE, SS.type = "III")
  fit.RRPP.alt <- rrpp.alt(y ~ x + x2/x3, data = df, SS.type = "III", Cov = Cov,
