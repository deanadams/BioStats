...
```{r echo=FALSE, out.width="80%" }
Y.pc<-prcomp(Y)$x
gp.col<-rep("red",nrow(Y.pc))
gp.col[which(bumpus$sex=="m")]<-"blue"
plot(Y.pc, asp=1,pch=21, bg=gp.col, cex=1.5,xlab="PC1", ylab="PC2")
pc.bird<-prcomp(Y)
...
plot(PCoA,pch=21,bg=gp.col,cex=1.5,asp=1)
```
...
######Statistical side-notes: $tr(\small\hat{\mathbf{\Sigma}}) = tr(\small\hat{\mathbf{D}})$.  Also, the number of PCoA dimensions with variation > 0% may be fewer than $\small{n}$ (and based on the number of original variables)

Note however, that if one had some other distance measure, a visualization via PCoA is possible.

###### NOTE: Can yield negative eigenvalues (if distances are semi-metric or nonmetric)
...
par(mfcol = c(1, 2))
plot(Y.pc, asp=1,pch=21, bg=gp.col, cex=1.5,xlab="PC1", ylab="PC2")
plot(Y.nmds$points, asp=1,pch=21, bg=gp.col, cex=1.5,xlab="NMDS1", ylab="NMDS2")
par(mfcol = c(1,1))
```
