...
anova(gls(TL~gp, correlation=corBrownian(phy=plethtree), data=data.frame(TL=TL,gp=gp)))  
...
A PCA with the phylogeny superimposed is a phylomorphospace. There are numerous functions for accomplishing this (e.g., in phytools): one we will illustrate is in geomorph (note that the ancestral states could be part of the output if anc.states = TRUE: 
